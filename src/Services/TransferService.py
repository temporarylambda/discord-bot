...
class TransferService:
    ...
    def transfer(self, from_user, to_user, amount, fee=15, note=None):
        ...
        self._transfer(transfer_reason_id=transfer_reason_id, to_user=to_user, amount=int(amount), fee=int(fee), note=note, from_user=from_user)
        return
    ...
    def _transfer(self, transfer_reason_id, to_user, amount: int, fee: int =0, note=None, from_user=None):
        UserRepositoryObject = UserRepository();

        # 扣款 - 如果有指定 from_user，則代表這筆錢是從某個人身上扣除，沒有了話則是從系統扣除，紀錄一筆從系統扣除的轉帳紀錄
        self.TransferRecordRepository.create(transfer_reason_id, from_user['id'] if from_user is not None else None, -amount, note)
        ...
        # 撥款
        self.TransferRecordRepository.create(transfer_reason_id, to_user['id'] if to_user is not None else None, amount - fee, note)
