...
| 指令 | 參數 | 說明 | 權限限制 |
|------|------|------|----------|
| `/invite 邀請` | 無 | 產出一則有時效性、一次性的邀請連結 | 一般使用者（不可封鎖） 
...
| 指令 | 參數 | 說明 | 權限限制 |
|------|------|------|----------|
| `/十八仔 開始賭局` | `amount: int`<BR>`rule: str` | 開始一場十八仔賭博 - 每人擲三顆骰子，根據 `rule` 參數決定最大或最小者贏 | 一般使用者（不可封鎖） 
...
| 指令 | 參數 | 說明 | 權限限制 |
|------|------|------|----------|
| `/不活躍用戶 列表` | 無 | 列出過去指定時間（參見 .env 檔案 RULE_INACTIVE_CHECKIN_DAYS 設定）內從未執行簽到的成員名單 | 僅限管理員（不可封鎖） 
